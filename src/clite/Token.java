...
public class Token {
    ...
    // make sure token is defined in TokenType
    private Token (TokenType t, String v) {
        ...
        if (t.compareTo(TokenType.Eof) < 0) {
            int ti = t.ordinal();
            reserved[ti] = v;
            //System.err.println("reserved[] " + reserved[ti]);
            token[ti] = this;
            //System.err.println("token[] " + token[ti]);
        }
    }
	...
	public static Token keyword(String name) {
		...
		// indent words that begin w a capital letter
		if (ch >= 'A' && ch <= 'Z') {
			//System.err.println("name " + name + "\n");
			return mkIdentTok(name);
		}

		// store keywords from .cpp file
		// ****idk if main needs to be in here and TokenType**** //
		for (int i = 0; i < KEYWORDS; i++) {
			if (name.equals(reserved[i])) {
				//System.err.println("okAndIn token[" + Integer.toString(i) + "]"+ token[i]);
				return token[i];
			}
		}
